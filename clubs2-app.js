...
export async function initClubs2() {
  ...
  function setMode(mode) {
    ...
    // Leaflet: when the container was hidden, the canvas size can be wrong.
    if (mode === "map" && MAP) {
      setTimeout(() => {
        try {
          MAP.invalidateSize();
  ...
  function initMap() {
    ...
    // Cluster plugin is optional; fall back to a normal layer group
    MAP_MARKERS = typeof L.markerClusterGroup === "function"
