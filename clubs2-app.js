...
export async function initClubs2() {
  ...
  function updateLayoutVars(){
    const fb = filtersBar;
    if (fb) document.documentElement.style.setProperty('--filters-h', `${fb.offsetHeight}px`);
  }
  ...
  function setMode(mode) {
    // ao trocar de vista, fecha o painel/drawer de detalhes
    closeDrawer();
    // Se a página não tiver modo list, ignora.
    if (mode === "list" && (!viewList || !listView)) mode = "grid";
    MODE = mode;
  ...
  // mantém vars de layout actualizadas (para sticky e painel)
  updateLayoutVars();
  ...
  clearBtn.addEventListener("click", () => {
    ...
    if (viewGrid) viewGrid.addEventListener("click", () => setMode("grid"));
    if (viewList) viewList.addEventListener("click", () => setMode("list"));
